...
Система парсинга электронных книг в форматах EPUB и FB2 с извлечением метаданных, содержимого глав, обложек и **CFI (Canonical Fragment Identifier) для epub.js интеграции**.

## 🎯 КРИТИЧЕСКОЕ ОБНОВЛЕНИЕ (23.10.2025)

**Новая функциональность:** Поддержка CFI (Canonical Fragment Identifier) для точной навигации в epub.js.

### Что добавлено:
- **CFI Generation** - автоматическая генерация 2000 location точек для каждой книги
- **epub.js compatibility** - полная интеграция с epub.js 0.3.93
- **Location-based progress** - точное отслеживание прогресса чтения (0-100%)
- **Hybrid restoration** - комбинация CFI + scroll offset для pixel-perfect позиционирования

## Архитектура парсера

### Основные принципы
- **Multi-format support** - EPUB 2.0/3.0, FB2.0/2.1
- **CFI support** - генерация locations для epub.js навигации (NEW)
