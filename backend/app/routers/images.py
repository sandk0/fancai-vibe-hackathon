...
from ..services.image_generator import image_generator_service, ImageGenerationRequest
...
async def generate_image_for_description(
    ...
    description_result = await db.execute(
        ...
        .where(Book.user_id == current_user.id)  # Проверяем что книга принадлежит пользователю
    )
...
async def generate_images_for_chapter(
    ...
    if request.description_types:
        descriptions_query = descriptions_query.where(Description.type.in_(request.description_types))
    
    descriptions_result = await db.execute(descriptions_query.order_by(Description.priority_score.desc()))
