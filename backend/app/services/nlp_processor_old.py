...
class SpacyProcessor(BaseNLPProcessor):
    ...
    def __init__(self):
        ...
        # Настраиваемые параметры (будут обновляться из настроек)
    ...
    def is_available(self) -> bool:
        """Проверяет доступность spaCy процессора."""
    ...
    def extract_descriptions(
        ...
        for i, sentence in enumerate(sentences):
            ...
            sentence_descriptions = self._analyze_sentence_spacy(sentence, i)
        ...
        print(f"✅ Extracted {len(filtered_descriptions)} descriptions using spaCy")
    ...
    def _analyze_sentence_spacy(
        ...
        for ent in doc.ents:
            if ent.label_ in ["LOC", "GPE"]:  # Локации
                descriptions.append(
                    {
                        "content": ent.text,
                        "context": sentence,
    ...
    def extract_descriptions_from_text(
        ...
        """
        print(
            f"🔍 extract_descriptions_from_text called for chapter {chapter_id}, text length: {len(text)}"
        )
                ...
                descriptions.append(
                    {
                        ...
                        "priority_score": self._calculate_priority_score(
                            ...
                            character_desc["confidence"],
                            len(sentence),
                        ),
                    }
                )

            atmosphere_desc = self._extract_atmosphere_description(
                sentence_doc, sentence
            )
            if atmosphere_desc:
                descriptions.append(
                    {
    ...
    def _get_context(
        self, sentences: List[str], position: int, context_size: int = 1
    ) -> str:
    ...
    def _calculate_priority_score(
        self, desc_type: DescriptionType, confidence: float, text_length: int
    ) -> float:
    ...
    def _extract_location_description(
        self, doc, sentence: str
        ...
        if confidence >= 0.4:
            return {"confidence": min(1.0, confidence), "entities": list(set(entities))}
    ...
    def _extract_character_description(
        ...
        for token in doc:
            if token.pos_ == "ADJ" and any(
                person_word in sentence.lower()
                for person_word in ["человек", "мужчина", "женщина"]
            ):
                confidence += 0.2
    ...
    def _extract_atmosphere_description(
        self, doc, sentence: str
    ) -> Optional[Dict[str, Any]]:
    ...
    def _extract_object_description(
        self, doc, sentence: str
    ) -> Optional[Dict[str, Any]]:
        ...
        for token in doc:
            if token.pos_ == "NOUN" and len(token.text) > 3:
                ...
                if not any(
                    person_word in token.text.lower()
                    for person_word in ["человек", "люд"]
                ):
                    confidence += 0.1
    ...
    try:
        ...
        for chapter in book.chapters:
            ...
            try:
                ...
                # )
                descriptions_data = []  # Placeholder - this file is deprecated
                ...
                for desc_data in descriptions_data:
                    description = Description(
                        ...
                        entities_mentioned=", ".join(
                            desc_data.get("entities_mentioned", [])
                        ),
                        emotional_tone=desc_data.get("emotional_tone", "neutral"),
                        complexity_level=desc_data.get("complexity_level", "medium"),
                        is_suitable_for_generation=desc_data.get("confidence_score", 0)
                        >= 0.3,
                    )
                    db.add(description)
        ...
        await db.rollback()
        return {"error": str(e), "total_descriptions": 0}


# Глобальный экземпляр процессора
# nlp_processor = NLPProcessor()  # Old code - disabled
