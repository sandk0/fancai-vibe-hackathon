...
class EnhancedNLPProcessor:
    ...
    def _update_metrics(
        self, results_count: int, processing_time: float, success: bool
    ):
        ...
        new_avg = (
            current_avg * (self.performance_metrics["total_processed"] - 1)
            + processing_time
        ...
        if success:
            current_success_count = self.performance_metrics["success_rate"] * (
                self.performance_metrics["total_processed"] - 1
            )
            new_success_rate = (current_success_count + 1) / self.performance_metrics[
                "total_processed"
            ]
    ...
    def _calculate_quality_score(self, descriptions: List[Dict[str, Any]]) -> float:
        """Вычисляет оценку качества извлеченных описаний (использует shared utility)."""
        return calculate_quality_score(descriptions)
...
class EnhancedSpacyProcessor(EnhancedNLPProcessor):
    ...
    def __init__(self, config: ProcessorConfig = None):
        ...
        self.spacy_config = self.config.custom_settings.get(
            "spacy",
            {
    ...
    async def _add_literary_patterns(self):
        ...
        character_patterns = [
            [
                {
                    "LOWER": {
                        "IN": ["молодой", "старый", "красивый", "высокий", "стройный"]
                    }
                },
    ...
    async def extract_descriptions(
        ...
        try:
            ...
            logger.info(
                f"SpaCy extracted {len(filtered_descriptions)} descriptions in {processing_time:.2f}s"
            )
    ...
    async def _extract_entity_descriptions(self, doc) -> List[Dict[str, Any]]:
        ...
        if not doc.ents:
            logger.info(
                "No entities found by NER, using fallback noun-adjective detection"
            )
        ...
        for ent in doc.ents:
            ...
            desc_type, base_confidence = self._map_entity_to_description_type(
                ent.label_
            )
            if not desc_type:
                continue
                ...
                descriptions.append(
                    {
                        ...
                        * entity_boosts.get(
                            f"{desc_type.lower()}_detection_boost", 1.0
                        ),
                        "source": "spacy_entity",
                    }
                )
    ...
    async def _extract_contextual_descriptions(self, doc) -> List[Dict[str, Any]]:
        ...
        for sent in doc.sents:
            ...
            atmosphere_score = self._calculate_atmosphere_score(
                sent, atmosphere_keywords
            )
    ...
    def _adjust_confidence_by_context(self, base_confidence: float, sentence) -> float:
        ...
        descriptive_count = sum(
            1 for token in sentence if token.pos_ in descriptive_pos
        )
    ...
    def _calculate_atmosphere_score(
        self, sentence, atmosphere_keywords: List[str]
    ) -> float:
        ...
        keyword_score = sum(
            0.2 for keyword in atmosphere_keywords if keyword in text_lower
        )
    ...
    def _filter_and_enhance_descriptions(
        ...
        for desc in unique_descriptions:
            if (
                ...
                and desc["word_count"] >= self.config.min_word_count
                and desc["confidence_score"] >= self.config.confidence_threshold
            ):
