...
class NLPModelCache:
    ...
    def get_model(self, model_name: str = "ru_core_news_lg") -> Language:
        ...
        try:
            ...
            model = spacy.load(
                model_name,
                disable=["lemmatizer", "textcat"]  # We don't need these
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "hit_rate": (total_hits / (total_hits + total_loads) * 100) if (total_hits + total_loads) > 0 else 0,
    ...
    def _update_stats(self, model_name: str, event: str, extra: Dict[str, Any] = None):
        ...
        if event == "hit":
            ...
            if extra and "load_time" in extra:
                stats["avg_load_time"] = (
                    (stats.get("avg_load_time", 0) * (stats["loads"] - 1) + extra["load_time"]) 
                    / stats["loads"]
                )
        elif event == "evict":
            stats["evictions"] += 1
