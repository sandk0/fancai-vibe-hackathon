...
class BatchProcessor:
    ...
    async def _flush_buffer(self, db: AsyncSession) -> int:
        ...
        try:
            ...
            logger.info(f"✅ Batch saved: {count} descriptions (Total: {self.total_saved})")
...
class ResourceMonitor:
    ...
    def should_pause(self) -> Tuple[bool, str]:
        ...
        if process_memory_mb > self.max_memory_mb:
            return True, f"Process memory ({process_memory_mb:.0f}MB) exceeds limit ({self.max_memory_mb}MB)"
                ...
                try:
                    ...
                    if descriptions:
                        await self.batch_processor.add_descriptions(
                            descriptions, 
                            db,
                            force_flush=(i == len(chapters) - 1)  # Flush on last chapter
                        )
                        total_descriptions += len(descriptions)
                    
                    # Update chapter status
                    await self._update_chapter_status(
                        db, 
                        chapter.id,
                        len(descriptions)
                    )
                    ...
                    if i % 5 == 0:
                        gc.collect()
                        logger.info(f"📊 Progress: {i+1}/{len(chapters)} chapters, "
...
# Export for use
__all__ = ['optimized_parser', 'OptimizedBookParser', 'BatchProcessor', 'ResourceMonitor']