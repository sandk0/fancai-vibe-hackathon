...
class PromptEngineer:
    ...
    def create_prompt(
        ...
        template = self.style_templates.get(
            description_type, self.style_templates[DescriptionType.LOCATION]
        )
        ...
        negative_key = (
            "character" if description_type == DescriptionType.CHARACTER else "default"
        )
        negative_prompt = self.negative_prompts.get(
            negative_key, self.negative_prompts["default"]
        )

        return {"positive": main_prompt, "negative": negative_prompt}
                ...
                async with aiohttp.ClientSession(timeout=self.timeout) as session:
                    async with session.get(url) as response:
                        if response.status == 200:
                            ...
                            logger.warning(
                                f"Generation attempt {attempt + 1} failed: {error_msg}"
                            )
...
class ImageGeneratorService:
    ...
    async def generate_image_for_description(
        self, description: Description, user_id: str, custom_style: Optional[str] = None
        ...
        prompts = self.prompt_engineer.create_prompt(
            description.content, description.type, custom_style
        )

        # Генерируем изображение
        result = await self.pollinations_client.generate_image(
            prompts["positive"], negative_prompt=prompts["negative"]
        )

        logger.info(
            f"Image generation result for description {description.id}: success={result.success}"
        )

        return result

    async def batch_generate_for_chapter(
        self, descriptions: List[Description], user_id: str, max_images: int = 5
        ...
        sorted_descriptions = sorted(
            descriptions, key=lambda d: d.priority_score, reverse=True
        )[:max_images]
        ...
        for desc in sorted_descriptions:
            try:
                ...
                logger.error(
                    f"Error generating image for description {desc.id}: {str(e)}"
                )
                results.append(
                    ImageGenerationResult(
    ...
    def add_to_queue(self, request: ImageGenerationRequest):
        ...
        logger.info(
            f"Added generation request to queue. Queue size: {len(self.generation_queue)}"
        )
