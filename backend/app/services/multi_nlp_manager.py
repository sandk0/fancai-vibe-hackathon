...
from .enhanced_nlp_system import EnhancedNLPProcessor, ProcessorConfig
...
class MultiNLPManager:
    ...
    async def initialize(self):
        ...
        async with self._init_lock:
            ...
            logger.info(
                f"✅ Multi-NLP Manager initialized with {len(self.processors)} processors"
            )
    ...
    async def _load_processor_configs(self):
        ...
        try:
            ...
            spacy_config = {
                ...
                "character_detection_boost": spacy_settings.get(
                    "character_detection_boost", 1.2
                ),
                "location_detection_boost": spacy_settings.get(
            ...
            self.processor_configs["spacy"] = ProcessorConfig(
                ...
                max_description_length=spacy_settings.get(
            ...
            self.processor_configs["natasha"] = ProcessorConfig(
                ...
                weight=natasha_settings.get(
                    "weight", 1.2
                ),  # Natasha лучше для русского
                confidence_threshold=natasha_settings.get("confidence_threshold", 0.4),
                min_description_length=natasha_settings.get(
                    "min_description_length", 40
                ),
                max_description_length=natasha_settings.get(
                    "max_description_length", 1200
                ),
                min_word_count=natasha_settings.get("min_word_count", 8),
                custom_settings={
                    "natasha": natasha_settings.get("natasha_specific", {})
                },
            )
    ...
    async def _get_processor_settings(self, processor_name: str) -> Dict[str, Any]:
        ...
        try:
            settings = await settings_manager.get_category_settings(
    ...
    async def _load_global_settings(self):
        ...
        try:
            ...
            self.processing_mode = ProcessingMode(
                global_settings.get("processing_mode", "single")
            )
    ...
    async def extract_descriptions(
        ...
        processors_to_use = self._select_processors(
    ...
    def _select_processors(
        ...
        if mode == ProcessingMode.SINGLE:
            ...
            return available_processors[
                : self.global_config.get("max_parallel_processors", 3)
            ]
    ...
    async def _process_single(
        self, text: str, chapter_id: str, processor_name: str
    ) -> ProcessingResult:
    ...
    async def _process_parallel(
        self, text: str, chapter_id: str, processor_names: List[str]
    ) -> ProcessingResult:
        ...
        for i, (name, _) in enumerate(tasks):
            if isinstance(results[i], Exception):
                ...
                quality_metrics[name] = self.processors[name]._calculate_quality_score(
                    descriptions
                )
    ...
    async def _process_sequential(
        ...
        for name in processor_names:
            if name in self.processors:
                try:
                    descriptions = await self.processors[name].extract_descriptions(
                        text, chapter_id
                    )
                    processor_results[name] = descriptions
                    quality_metrics[name] = self.processors[
                        name
                    ]._calculate_quality_score(descriptions)
                    all_descriptions.extend(descriptions)
                except Exception as e:
    ...
    async def _process_ensemble(
        ...
        parallel_result = await self._process_parallel(
            text, chapter_id, processor_names
        )
    ...
    def _combine_descriptions(
        ...
        for group_descriptions in grouped.values():
            ...
            best_desc["consensus_strength"] = len(group_descriptions) / len(
                self.processors
            )
    ...
    def _ensemble_voting(
        self, processor_results: Dict[str, List[Dict[str, Any]]]
    ) -> List[Dict[str, Any]]:
    ...
    def _estimate_text_complexity(self, text: str) -> float:
        ...
        # Нормализуем метрики
        word_complexity = min(
            1.0, avg_word_length / 10
        )  # Слова >10 символов считаются сложными
        sentence_complexity = min(
            1.0, avg_sentence_length / 20
        )  # Предложения >20 слов сложные
